...
function [weights,sphere] = myOrica(data,varargin)
disp('data');
disp(data(1,1));
if nargin < 1
    help orica  
...
adaptiveFF.profile = 'cooling'; %CHANGE BACK TO cooling
...
if ~onlineWhitening     % pre-whitening
    ...
    state.icasphere = 2.0*inv(sqrtm(double(cov(data')))); %#ok<MINV> % find the "sphering" matrix = spher()
